      ...
      <div className="mt-6 rounded-3xl border border-slate-800 bg-slate-900 p-8">
        ...
        {isDemoMode ? (
          <p className="mt-4 text-sm text-slate-400">
            Demo mode is active here, so booking and verification actions validate and redirect but do not persist yet.
          </p>
        ) : null}
      </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {data.booking.verificationStatus !== "REJECTED" ? (
                <form action={updateVerificationStatus} className="flex w-full flex-col gap-3 rounded-xl border border-slate-800 bg-slate-950/60 p-4 sm:w-auto">
                  <input type="hidden" name="bookingId" value={data.booking.id} />
                  <input type="hidden" name="nextStatus" value="REJECTED" />
                  <input type="hidden" name="returnTo" value={`/admin/bookings/${data.booking.id}`} />
                  <label className="text-xs uppercase tracking-wide text-slate-500">
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Optional context for why verification was rejected. This will show in the booking timeline."
                      className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-rose-400"
                    />
                  </label>
                  <button
